...
const hook = async () => {
    ...
    const config = new core_1.Config({ name: (0, ts_types_1.get)(pjson, 'oclif.bin'), root });
    await config.load();
    ...
    core_1.CliUx.ux.log(`user plugins are ${installedUserPlugins.join(', ')}`);
    if (installedUserPlugins.includes('salesforcedx')) {
        core_1.CliUx.ux.warn(salesforcedxError);
    }
};
