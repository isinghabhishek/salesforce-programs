...
class PackageVersionListCommand extends command_1.SfdxCommand {
    async run() {
        ...
        const pv = new packaging_1.PackageVersion({ connection: this.hubOrg.getConnection(), project: undefined });
        const packageIdOrAliases = this.flags.packages || [];
        // resolve any passed in values from aliases or IDs
        const packages = packageIdOrAliases.map((p) => { var _a; return (_a = (0, packaging_1.getPackageIdFromAlias)(p, project)) !== null && _a !== void 0 ? _a : p; });
        ...
        if ((records === null || records === void 0 ? void 0 : records.length) > 0) {
            ...
            if ((ancestorIds === null || ancestorIds === void 0 ? void 0 : ancestorIds.length) > 0) {
                ancestorVersionsMap = await (0, packaging_1.getPackageVersionStrings)(ancestorIds, this.hubOrg.getConnection());
            }
            ...
            const containerOptionsMap = await (0, packaging_1.getContainerOptions)(recordIds, this.hubOrg.getConnection());
            records.forEach((record) => {
                const ids = [record.Id, record.SubscriberPackageVersionId];
                const aliases = [];
                ids.forEach((id) => {
                    const matches = (0, packaging_1.getPackageAliasesFromId)(id, project);
                    if (matches.length > 0) {
