import { SfdxCommand } from '@salesforce/command';
export declare type PackageInstalledListResult = {
    Id: string;
    SubscriberPackageId: string;
    SubscriberPackageName: string;
    SubscriberPackageNamespace: string;
    SubscriberPackageVersionId: string;
    SubscriberPackageVersionName: string;
    SubscriberPackageVersionNumber: string;
};
export declare class PackageInstalledListCommand extends SfdxCommand {
    static readonly description: string;
    static readonly examples: string[];
    static readonly requiresUsername = true;
    static readonly requiresProject = true;
    run(): Promise<PackageInstalledListResult[]>;
}
