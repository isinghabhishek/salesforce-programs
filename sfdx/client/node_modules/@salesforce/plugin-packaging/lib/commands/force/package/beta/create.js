...
const packageVersionCreate = core_1.Messages.loadMessages('@salesforce/plugin-packaging', 'package_version_create');
class PackageCreateCommand extends command_1.SfdxCommand {
    async run() {
        ...
        const result = await (0, packaging_1.createPackage)(this.hubOrg.getConnection(), this.project, options).catch((err) => {
            // TODO: until package2 is GA, wrap perm-based errors w/ 'contact sfdc' action (REMOVE once package2 is GA'd)
            err = (0, packaging_1.massageErrorMessage)(err);
            throw (0, packaging_1.applyErrorAction)(err);
        });
        this.display(result);
        return result;
    }
