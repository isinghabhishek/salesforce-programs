...
const messages = core_1.Messages.loadMessages('@salesforce/plugin-env', 'resume.sandbox');
class ResumeSandbox extends sandboxCommandBase_1.SandboxCommandBase {
    ...
    async resumeSandbox() {
        ...
        if (await this.verifyIfAuthExists(this.prodOrg, this.sandboxRequestData.sandboxProcessObject.SandboxName, this.flags['job-id'], lifecycle)) {
    ...
    buildSandboxRequestCacheEntry() {
        ...
        if (this.flags['use-most-recent']) {
            ...
            const sce = this.sandboxRequestConfig
                .entries()
                .find(([, e]) => e.sandboxProcessObject.Id === this.flags['job-id'])[1];
            sandboxRequestCacheEntry = sce || sandboxRequestCacheEntry;
        }
        sandboxRequestCacheEntry.prodOrgUsername ?? (sandboxRequestCacheEntry.prodOrgUsername = this.flags['target-org']?.getUsername());
        (_a = sandboxRequestCacheEntry.sandboxProcessObject).SandboxName ?? (_a.SandboxName = this.flags.name);
        return sandboxRequestCacheEntry;
    }
    async verifyIfAuthExists(prodOrg, sandboxName, jobId, lifecycle) {
        const sandboxProcessObject = await this.getSandboxProcessObject(prodOrg, sandboxName, jobId);
    ...
    async getSandboxProcessObject(prodOrg, sandboxName, jobId) {
